...
If (and only if) the gate passes, invoke the **developer** agent (model: sonnet, pass this explicitly on the Agent tool call, see `.claude/agents/developer.md`) on the story identified below. It implements the slice against the feature prototype and architecture (reuse first per `docs/components.md`, Tailwind variant rules, tests from the QA plan alongside the code), updates `docs/components.md` for shared component changes, records deviations, runs tests and linting, and sets the story status to `verifying`.
