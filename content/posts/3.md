...
How can we change a property's type based on another property's value? For instance, return a single item by default, and an array of items when `multiple` is set. To demonstrate what I mean, see the recording below. Here, you can observe how the type changes based on the `multiple` attribute.
...
Another example is the BlogPostContext.vue file within this [website project](https://github.com/jeroenbach/bach.software) (the reason for this article), where I return a single post when the ID is specified and all posts when it's not.
