...
slug: produktionsreifer-kubernetes-cluster-azure-terraform
...
> **Update (Dez 2025):** Nach dem Betrieb dieser Plausible-Bereitstellung für einige Monate in der minimalen Azure-Umgebung
> bin ich auf Speicherengpässe und Festplattenprobleme gestoßen, die dazu führten, dass die Container wiederholt abstürzten.
> Die beiden zusätzlichen Abschnitte [Hürde: Speicherengpass für clickhouse Container](#hürde-speicherengpass-für-clickhouse-container)
> und [Hürde: Festplatte voll aufgrund von Systemlogs](#hürde-festplatte-voll-aufgrund-von-systemlogs) beschreiben die Grundursachen und die Konfigurationsänderungen, die diese Stabilitätsprobleme behoben haben.
...
### Hürde: Speicherengpass für clickhouse Container

Der ClickHouse-Container verwendete zu viel Speicher, was dazu führte, dass die Plausible-Lösung alle paar Stunden abstürzte.
Dies war auf die interne Konfiguration von ClickHouse zurückzuführen, es prüft den verfügbaren Speicher und setzt seine eigenen Limits automatisch auf 75% davon.

Das Problem war, dass ClickHouse den gesamten Speicher des Knotens (4GB) betrachtete und versuchte, 75% davon (3GB) zu verwenden, was zu Speicherproblemen für die anderen Pods führte, die auf demselben Knoten liefen.

**Die Lösung:** Setzen Sie ein Kubernetes-Speicherlimit für den ClickHouse-Container. Wenn Kubernetes ein Speicherlimit setzt, wird dieses über cgroups an den Container übergeben. ClickHouse liest dieses cgroup-Limit anstelle des gesamten Knotenspeichers und berechnet 75% des Container-Limits anstatt 75% des Knotenspeichers.

Durch das Setzen eines Limits von 1229Mi (ungefähr 1.2Gi) sieht ClickHouse jetzt nur noch diese Menge und verwendet 75% von 1229Mi (≈922Mi), was es gut innerhalb akzeptabler Grenzen hält und gleichzeitig genug Speicher für andere Pods übrig lässt.
...
clickhouse:
  resources:
    limits:
      memory: 1229Mi  # 1.2Gi max um innerhalb der Knotenkapazität zu bleiben
```

::

### Hürde: Festplatte voll aufgrund von Systemlogs

Nach einigen Monaten Betrieb von Plausible stellte ich fest, dass die ClickHouse-Datenbank **7.5GB Festplattenspeicher** allein für Systemlogs verbraucht hatte. Dies füllte schnell die zugewiesene Festplatte und führte zu Abstürzen der Container.

ClickHouse aktiviert standardmäßig mehrere umfangreiche Systemlogs:
- **trace_log**: Verwendete 2.6GB
- **metric_log**: Verwendete 1.9GB
- **asynchronous_metric_log**: Verwendete 1.9GB

Für ein kleines Analytics-Setup wie unseres sind diese detaillierten Systemlogs unnötig und verschwenderisch.

**Die Lösung:** Die umfangreichen Systemlogs vollständig deaktivieren und nur wesentliche Logs mit kurzen Aufbewahrungszeiten behalten. Ich verwendete die `extraOverrides`-Konfiguration im ClickHouse Helm-Chart, um eine benutzerdefinierte XML-Konfiguration einzufügen:
...
clickhouse:
  ...
  extraOverrides: |
    <clickhouse>
      <!-- trace_log deaktivieren (verwendete 2.6GB) -->
      <trace_log remove="1"/>
      <!-- metric_log deaktivieren (verwendete 1.9GB) -->
      <metric_log remove="1"/>
      <!-- asynchronous_metric_log deaktivieren (verwendete 1.9GB) -->
      <asynchronous_metric_log remove="1"/>
      <!-- query_log behalten, aber mit kurzer 7-Tage-Aufbewahrung -->
      <query_log>
      ...
      <!-- text_log behalten, aber mit kurzer 3-Tage-Aufbewahrung -->
      <text_log>
        <database>system</database>
...
Diese Konfiguration reduzierte die Festplattennutzung erheblich, während die wesentlichen Abfragelogs (mit 7-tägiger Aufbewahrung) und Textlogs (mit 3-tägiger Aufbewahrung) für Debugging-Zwecke bei Bedarf erhalten blieben.
