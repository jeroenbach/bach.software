...
> **Update (Dec 2025):** After running this Plausible deployment for a few months on the minimal Azure environment,
> I encountered memory shortages and disk space issues that caused the containers to crash repeatedly.
> The two additional sections [Hurdle: Memory shortage for clickhouse container](#hurdle-memory-shortage-for-clickhouse-container) 
and [Hurdle: Disk full due to system logs](#hurdle-disk-full-due-to-system-logs) describe the root causes and the configuration changes that resolved these stability issues. 
...
### Hurdle: Memory shortage for clickhouse container

The ClickHouse container was using too much memory, causing the Plausible solution to crash every couple of hours.
This was due to ClickHouse's internal configuration, it looks at the available memory and automatically sets its own limits to 75% of it.

The problem was that ClickHouse was looking at the entire node's memory (4GB) and trying to use 75% of that (3GB), which caused memory issues for the other pods running on the same node.

**The solution:** Set a Kubernetes memory limit for the ClickHouse container. When Kubernetes sets a memory limit, this is passed to the container through cgroups. ClickHouse reads this cgroup limit instead of the node's total memory, and calculates 75% of the container limit rather than 75% of the node's memory.

By setting a limit of 1229Mi (approximately 1.2Gi), ClickHouse now sees only this amount and uses 75% of 1229Mi (≈922Mi), which keeps it well within acceptable bounds while leaving enough memory for other pods.
...
### Hurdle: Disk full due to system logs

After running Plausible for a few months, I discovered that the ClickHouse database had consumed **7.5GB of disk space** just for system logs. This quickly filled up the allocated disk and caused the containers to crash.

ClickHouse by default enables several heavy system logs:
- **trace_log**: Was using 2.6GB
- **metric_log**: Was using 1.9GB
- **asynchronous_metric_log**: Was using 1.9GB

For a small analytics setup like ours, these detailed system logs are unnecessary and wasteful.

**The solution:** Disable the heavy system logs entirely and keep only essential logs with short retention periods. I used the `extraOverrides` configuration in the ClickHouse Helm chart to inject custom XML configuration:
...
This configuration dramatically reduced disk usage while still maintaining the essential query logs (with 7-day retention) and text logs (with 3-day retention) for debugging purposes when needed.
