...
dateModified: 2025-09-27T21:00:00
...
export function toMap<
  T,
  K extends keyof T | ((item: T) => any) | undefined,
...
>(
  ...
  // Either use the key as a function directly, or create a function that uses the keyof
  type TKeyType = undefined extends K ? T : K extends ((item: T) => infer KF) ? KF : K extends keyof T ? T[K] : never;
  const getKeyFn: (item: T) => TKeyType
    = key === undefined
      ? item => item
      : typeof key === 'function'
        ? key
        : item => typeof item === 'object' && item !== null ? item[key as keyof T] : item;

  // In case of the value, the param can also be undefined,
  // in that case we return the whole item
  type TValueType = undefined extends V ? T : V extends ((item: T) => infer VF) ? VF : V extends keyof T ? T[V] : never;
  const getValueFn: (item: T) => TValueType
    = value === undefined
      ? item => item
      : typeof value === 'function'
        ? value
        : item => typeof item === 'object' && item !== null ? item[value as keyof T] : item;

  // Create the map and loop the array only once
  const map = new Map<TKeyType, TValueType>();
  array?.forEach((item) => {
    map.set(getKeyFn(item), getValueFn(item));
  });
...
// ---cut---
// Make the function more flexible - optional parameters give you more power  // [!code highlight]
...
Want to see how thoroughly this function is tested? Check out the full test suite here [toMap.tests.ts](https://github.com/jeroenbach/bach.software/blob/main/src/app/utils/toMap.test.ts).
