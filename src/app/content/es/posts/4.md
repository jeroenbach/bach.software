...
dateModified: 2025-12-06T21:00:00
imageUrl: /images/posts/4/cover.jpeg
...
> **Actualización (Dic 2025):** Después de ejecutar este despliegue de Plausible durante algunos meses en el entorno mínimo de Azure,
> encontré escasez de memoria y problemas de espacio en disco que causaron que los contenedores se bloquearan repetidamente.
> Las dos secciones adicionales [Obstáculo: Escasez de memoria para el contenedor clickhouse](#obstáculo-escasez-de-memoria-para-el-contenedor-clickhouse)
> y [Obstáculo: Disco lleno debido a logs del sistema](#obstáculo-disco-lleno-debido-a-logs-del-sistema) describen las causas raíz y los cambios de configuración que resolvieron estos problemas de estabilidad.
...
### Obstáculo: Escasez de memoria para el contenedor clickhouse

El contenedor ClickHouse estaba usando demasiada memoria, causando que la solución Plausible se bloqueara cada pocas horas.
Esto se debía a la configuración interna de ClickHouse, examina la memoria disponible y establece automáticamente sus propios límites al 75% de ella.

El problema era que ClickHouse estaba examinando la memoria total del nodo (4GB) e intentando usar el 75% de eso (3GB), lo que causaba problemas de memoria para los otros pods que se ejecutaban en el mismo nodo.

**La solución:** Establecer un límite de memoria de Kubernetes para el contenedor ClickHouse. Cuando Kubernetes establece un límite de memoria, esto se pasa al contenedor a través de cgroups. ClickHouse lee este límite de cgroup en lugar de la memoria total del nodo, y calcula el 75% del límite del contenedor en lugar del 75% de la memoria del nodo.

Al establecer un límite de 1229Mi (aproximadamente 1.2Gi), ClickHouse ahora solo ve esta cantidad y usa el 75% de 1229Mi (≈922Mi), lo que lo mantiene dentro de límites aceptables mientras deja suficiente memoria para otros pods.

::code-group

```yaml [modules/plausible/plausible.tf]
clickhouse:
  resources:
    limits:
      memory: 1229Mi  # 1.2Gi max para permanecer dentro de la capacidad del nodo
```

::

### Obstáculo: Disco lleno debido a logs del sistema

Después de ejecutar Plausible durante algunos meses, descubrí que la base de datos ClickHouse había consumido **7.5GB de espacio en disco** solo para logs del sistema. Esto llenó rápidamente el disco asignado y causó bloqueos de los contenedores.

ClickHouse habilita por defecto varios logs del sistema pesados:
- **trace_log**: Estaba usando 2.6GB
- **metric_log**: Estaba usando 1.9GB
- **asynchronous_metric_log**: Estaba usando 1.9GB

Para una configuración de análisis pequeña como la nuestra, estos logs del sistema detallados son innecesarios y despilfarradores.

**La solución:** Deshabilitar completamente los logs del sistema pesados y mantener solo los logs esenciales con períodos cortos de retención. Usé la configuración `extraOverrides` en el chart de Helm de ClickHouse para inyectar configuración XML personalizada:
...
clickhouse:
  ...
  extraOverrides: |
    <clickhouse>
      <!-- Deshabilitar trace_log (estaba usando 2.6GB) -->
      <trace_log remove="1"/>
      <!-- Deshabilitar metric_log (estaba usando 1.9GB) -->
      <metric_log remove="1"/>
      <!-- Deshabilitar asynchronous_metric_log (estaba usando 1.9GB) -->
      <asynchronous_metric_log remove="1"/>
      <!-- Mantener query_log pero con retención corta de 7 días -->
      <query_log>
      ...
      <!-- Mantener text_log pero con retención corta de 3 días -->
      <text_log>
        <database>system</database>
...
Esta configuración redujo dramáticamente el uso del disco mientras mantenía los logs de consultas esenciales (con retención de 7 días) y los logs de texto (con retención de 3 días) para propósitos de depuración cuando sea necesario.
