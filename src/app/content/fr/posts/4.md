...
> **Mise à jour (Déc 2025):** Après avoir exécuté ce déploiement Plausible pendant quelques mois sur l'environnement Azure minimal,
> j'ai rencontré des pénuries de mémoire et des problèmes d'espace disque qui ont provoqué des crashs répétés des conteneurs.
> Les deux sections supplémentaires [Obstacle: Pénurie de mémoire pour le conteneur clickhouse](#obstacle-pénurie-de-mémoire-pour-le-conteneur-clickhouse)
> et [Obstacle: Disque plein à cause des logs système](#obstacle-disque-plein-à-cause-des-logs-système) décrivent les causes profondes et les modifications de configuration qui ont résolu ces problèmes de stabilité.
...
### Obstacle: Pénurie de mémoire pour le conteneur clickhouse

Le conteneur ClickHouse utilisait trop de mémoire, provoquant des crashs répétés de la solution Plausible toutes les quelques heures.
Cela était dû à la configuration interne de ClickHouse, il examine la mémoire disponible et définit automatiquement ses propres limites à 75% de celle-ci.

Le problème était que ClickHouse examinait la mémoire totale du nœud (4GB) et essayait d'utiliser 75% de cela (3GB), ce qui causait des problèmes de mémoire pour les autres pods s'exécutant sur le même nœud.

**La solution:** Définir une limite de mémoire Kubernetes pour le conteneur ClickHouse. Lorsque Kubernetes définit une limite de mémoire, celle-ci est transmise au conteneur via cgroups. ClickHouse lit cette limite cgroup au lieu de la mémoire totale du nœud, et calcule 75% de la limite du conteneur plutôt que 75% de la mémoire du nœud.

En définissant une limite de 1229Mi (environ 1.2Gi), ClickHouse ne voit maintenant que cette quantité et utilise 75% de 1229Mi (≈922Mi), ce qui le maintient bien dans des limites acceptables tout en laissant suffisamment de mémoire pour les autres pods.
...
clickhouse:
  resources:
    limits:
      memory: 1229Mi  # 1.2Gi max pour rester dans la capacité du nœud
```

::

### Obstacle: Disque plein à cause des logs système

Après avoir exécuté Plausible pendant quelques mois, j'ai découvert que la base de données ClickHouse avait consommé **7.5GB d'espace disque** rien que pour les logs système. Cela a rapidement rempli le disque alloué et a provoqué des crashs des conteneurs.

ClickHouse active par défaut plusieurs logs système lourds:
- **trace_log**: Utilisait 2.6GB
- **metric_log**: Utilisait 1.9GB
- **asynchronous_metric_log**: Utilisait 1.9GB

Pour une petite installation d'analyse comme la nôtre, ces logs système détaillés sont inutiles et gaspilleurs.

**La solution:** Désactiver complètement les logs système lourds et conserver uniquement les logs essentiels avec de courtes périodes de rétention. J'ai utilisé la configuration `extraOverrides` dans le chart Helm ClickHouse pour injecter une configuration XML personnalisée:
...
clickhouse:
  ...
  extraOverrides: |
    <clickhouse>
      <!-- Désactiver trace_log (utilisait 2.6GB) -->
      <trace_log remove="1"/>
      <!-- Désactiver metric_log (utilisait 1.9GB) -->
      <metric_log remove="1"/>
      <!-- Désactiver asynchronous_metric_log (utilisait 1.9GB) -->
      <asynchronous_metric_log remove="1"/>
      <!-- Conserver query_log mais avec une rétention courte de 7 jours -->
      <query_log>
      ...
      <!-- Conserver text_log mais avec une rétention courte de 3 jours -->
      <text_log>
        <database>system</database>
...
Cette configuration a considérablement réduit l'utilisation du disque tout en conservant les logs de requêtes essentiels (avec une rétention de 7 jours) et les logs texte (avec une rétention de 3 jours) à des fins de débogage si nécessaire.
