...
> **Update (Dec 2025):** Na het draaien van deze Plausible implementatie voor een paar maanden op de minimale Azure omgeving,
> liep ik tegen geheugentekorten en schijfruimteproblemen aan die ervoor zorgden dat de containers herhaaldelijk crashten.
> De twee aanvullende secties [Obstakel: Geheugentekort voor clickhouse container](#obstakel-geheugentekort-voor-clickhouse-container)
> en [Obstakel: Schijf vol door systeemlogboeken](#obstakel-schijf-vol-door-systeemlogboeken) beschrijven de hoofdoorzaken en de configuratiewijzigingen die deze stabiliteitsproblemen oplosten.
...
Als je de Cloudflare variabelen niet opgeeft, wordt de DNS niet bijgewerkt, maar al het andere zal nog steeds werken en je krijgt aan het einde het IP-adres te zien (om Plausible mee te benaderen).
...
### Obstakel: Geheugentekort voor clickhouse container

De ClickHouse container gebruikte te veel geheugen, waardoor de Plausible oplossing elke paar uur crashte.
Dit was te wijten aan de interne configuratie van ClickHouse, het kijkt naar het beschikbare geheugen en stelt automatisch zijn eigen limieten in op 75% daarvan.

Het probleem was dat ClickHouse keek naar het totale geheugen van de node (4GB) en probeerde 75% daarvan (3GB) te gebruiken, wat geheugenproblemen veroorzaakte voor de andere pods die op dezelfde node draaiden.

**De oplossing:** Stel een Kubernetes geheugenlimiet in voor de ClickHouse container. Wanneer Kubernetes een geheugenlimiet instelt, wordt dit doorgegeven aan de container via cgroups. ClickHouse leest deze cgroup limiet in plaats van het totale geheugen van de node, en berekent 75% van de container limiet in plaats van 75% van het geheugen van de node.

Door een limiet van 1229Mi (ongeveer 1.2Gi) in te stellen, ziet ClickHouse nu alleen deze hoeveelheid en gebruikt 75% van 1229Mi (≈922Mi), wat het goed binnen acceptabele grenzen houdt terwijl er voldoende geheugen overblijft voor andere pods.
...
clickhouse:
  resources:
    limits:
      memory: 1229Mi  # 1.2Gi max om binnen de node capaciteit te blijven
```

::

### Obstakel: Schijf vol door systeemlogboeken

Na het draaien van Plausible voor een paar maanden, ontdekte ik dat de ClickHouse database **7.5GB schijfruimte** had verbruikt alleen al voor systeemlogboeken. Dit vulde snel de toegewezen schijf en veroorzaakte crashes van de containers.

ClickHouse schakelt standaard verschillende zware systeemlogboeken in:
- **trace_log**: Gebruikte 2.6GB
- **metric_log**: Gebruikte 1.9GB
- **asynchronous_metric_log**: Gebruikte 1.9GB

Voor een kleine analytics setup zoals de onze zijn deze gedetailleerde systeemlogboeken onnodig en verspillend.

**De oplossing:** Schakel de zware systeemlogboeken volledig uit en behoud alleen essentiële logs met korte bewaartermijnen. Ik gebruikte de `extraOverrides` configuratie in de ClickHouse Helm chart om aangepaste XML configuratie te injecteren:
...
clickhouse:
  ...
  extraOverrides: |
    <clickhouse>
      <!-- trace_log uitschakelen (gebruikte 2.6GB) -->
      <trace_log remove="1"/>
      <!-- metric_log uitschakelen (gebruikte 1.9GB) -->
      <metric_log remove="1"/>
      <!-- asynchronous_metric_log uitschakelen (gebruikte 1.9GB) -->
      <asynchronous_metric_log remove="1"/>
      <!-- query_log behouden maar met korte 7-dagen retentie -->
      <query_log>
      ...
      <!-- text_log behouden maar met korte 3-dagen retentie -->
      <text_log>
        <database>system</database>
...
Deze configuratie verminderde het schijfgebruik dramatisch terwijl de essentiële query logs (met 7-dagen retentie) en text logs (met 3-dagen retentie) behouden bleven voor debugging doeleinden indien nodig.
